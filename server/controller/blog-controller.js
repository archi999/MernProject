...
const addBlog = async (req, res, next) => {
    ...
    const { title, desc, img, user, comment } = req.body;
    ...
    const blog = new Blog({
        title, desc, img, user, comment, date: currentDate
    });
...
const updateBlog = async (req, res, next) => {
    ...
    const blogObjectId = mongoose.Types.ObjectId(blogId);

    let blog={};

    try {
        blog = await Blog.findByIdAndUpdate({_id:blogId},{ $set: { title, desc, img } },
            { new: true })
